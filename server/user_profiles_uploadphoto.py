...
from server.general import decode_token, check_http_status, ValueError, id_to_user, authorise_token, invalid_dimension
...
def user_profiles_uploadphoto(token, img_url, x_start, y_start, x_end, y_end):
    if not check_http_status(img_url):
        raise ValueError(description="Invalid image url (JPEG only)")
    else:
        ...
        if width < x_end or height < y_end or invalid_dimension(x_start, y_start, x_end, y_end):
            #"x_start, y_start, x_end, y_end aren't all within the dimensions of the image"
            raise ValueError(description="Invalid photo dimensions")
        else:
