# move outside of server to run tests 
...
def test_admin_userpermission_change():
    ...
    assert "User is not an admin or owner" in str(excinfo.value)
    ...
    assert "User is not registered" in str(excinfo.value)

    # permission_id is not a valid id 
    with pytest.raises(ValueError) as excinfo:
        admin_userpermission_change(token1, 10, 4)
    assert "Invalid permission" in str(excinfo.value)
